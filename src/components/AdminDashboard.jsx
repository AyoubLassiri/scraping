...
import { Trash2, Plus, LogOut, Package, ShoppingCart, Edit, X, Users, Shield } from "lucide-react";

export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState("orders"); // orders | products | staff | players
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [ordersRes, productsRes, staffRes, playersRes] = await Promise.all([
      ...
      const [ordersData, productsData, staffData, playersData] = await Promise.all([
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-neutral-50 text-neutral-900 font-sans">
      <header className="bg-white border-b border-neutral-200 px-6 py-4 flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 text-xs font-semibold text-red-600 hover:text-red-700 bg-red-50 px-4 py-2 rounded-lg transition-colors"
        >
          <LogOut size={15} />
          تسجيل الخروج
        </button>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* Navigation Tabs */}
        <div className="flex gap-4 border-b border-neutral-200 mb-8 pb-3 overflow-x-auto">
          <button
            onClick={() => setActiveTab("orders")}
            className={`flex items-center gap-2 text-sm font-semibold pb-3 border-b-2 transition-colors whitespace-nowrap ${
              activeTab === "orders" ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 text-sm font-semibold pb-3 border-b-2 transition-colors whitespace-nowrap ${
              activeTab === "products" ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-400"
            }`}
          >
            <Package size={18} />
          ...
          <button
            ...
            className={`flex items-center gap-2 text-sm font-semibold pb-3 border-b-2 transition-colors whitespace-nowrap ${
              activeTab === "staff" ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-400"
            }`}
          >
            <Users size={18} />
          ...
          <button
            ...
            className={`flex items-center gap-2 text-sm font-semibold pb-3 border-b-2 transition-colors whitespace-nowrap ${
              activeTab === "players" ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-400"
            }`}
          >
            <Shield size={18} />
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <div key={order.id} className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm flex flex-col md:flex-row justify-between gap-6">
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-bold bg-neutral-900 text-white px-2.5 py-1 rounded">طلب #{order.id}</span>
                    ...
                    <p className="text-xs text-neutral-600">الهاتف: <span className="font-semibold">{order.phone}</span></p>
                    <p className="text-xs text-neutral-600">العنوان: {order.address}, {order.city}</p>
                  </div>

                  <div className="bg-neutral-50 p-4 rounded-lg md:w-1/2 flex flex-col justify-between">
                    <h4 className="text-xs font-bold text-neutral-800 mb-2">المنتجات المطلوبة:</h4>
                    ...
                    <div className="border-t border-neutral-200 mt-3 pt-2 flex justify-between text-xs font-bold">
                      <span>المجموع الكلي:</span>
                      <span className="text-emerald-600">{order.total_price} درهم</span>
                    </div>
                  </div>
                </div>
              ))
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm h-fit">
              <div className="flex justify-between items-center mb-4">
                ...
                {editingId && (
                  <button onClick={handleCancelEdit} className="text-xs text-neutral-500 hover:text-neutral-900 flex items-center gap-1">
              ...
              <form onSubmit={handleSubmitProduct} className="flex flex-col gap-3">
                <input
                  ...
                  required
                  className="w-full border border-neutral-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-neutral-900"
                />
                <input
                  type="number"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    ...
                    className="w-full border border-neutral-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-neutral-900 bg-white"
                  >
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-neutral-600 mb-1">صور المنتج (اختر عدة صور)</label>
                  <input
                    type="file"
                    multiple
                    accept="image/*"
                    onChange={handleImageChange}
                    className="w-full border border-neutral-200 rounded-lg p-2 text-xs file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-neutral-900 file:text-white hover:file:bg-neutral-800 cursor-pointer"
                  />
                  
                  {imagePreviews.length > 0 && (
                    <div className="flex gap-2 mt-2 flex-wrap">
                      {imagePreviews.map((src, index) => (
                        <div key={index} className="w-14 h-14 bg-neutral-100 rounded border border-neutral-200 overflow-hidden relative shadow-sm group">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  />
                  <label htmlFor="inStockCheck" className="text-xs text-neutral-700 cursor-pointer">المنتج متوفر في المخزون</label>
                </div>

                <button
                  type="submit"
                  className="mt-3 flex items-center justify-center gap-2 bg-neutral-900 text-white py-2.5 rounded-lg text-xs font-semibold hover:bg-neutral-800 transition-colors"
                >
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                {products.map((product) => {
                  ...
                  return (
                    <div key={product.id} className="flex items-center justify-between p-3 border border-neutral-100 rounded-lg hover:bg-neutral-50">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-neutral-100 rounded flex items-center justify-center overflow-hidden shrink-0">
                          {imgUrl && <img src={imgUrl} alt={product.name} className="w-full h-full object-contain mix-blend-multiply" />}
                        </div>
                        <div>
                          <h4 className="text-xs font-semibold text-neutral-900">{product.name}</h4>
                          <div className="flex items-center gap-2">
                            <p className="text-[11px] text-neutral-500">{product.price} درهم</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm h-fit">
              <div className="flex justify-between items-center mb-4">
                ...
                {editingStaffId && (
                  <button onClick={handleCancelStaffEdit} className="text-xs text-neutral-500 hover:text-neutral-900 flex items-center gap-1">
                    <X size={14} /> إلغاء التعديل
                  </button>
                )}
              </div>
              ...
              <form onSubmit={handleSubmitStaff} className="flex flex-col gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-neutral-600 mb-1">صورة عضو الطاقم</label>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  />
                  {staffImagePreview && (
                    <div className="w-14 h-14 bg-neutral-100 rounded-full border border-neutral-200 overflow-hidden mt-2 relative shadow-sm">
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                {staffList.length === 0 ? (
                  ...
                  staffList.map((staff) => (
                    <div key={staff.id} className="flex items-center justify-between p-4 border border-neutral-100 rounded-lg hover:bg-neutral-50">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-neutral-100 rounded-full flex items-center justify-center overflow-hidden shrink-0 border border-neutral-200">
                          {staff.image ? (
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="bg-neutral-900 text-white text-[10px] px-2 py-0.5 rounded font-medium">{staff.role}</span>
                          </div>
                          {staff.description && <p className="text-[11px] text-neutral-500 mt-1">{staff.description}</p>}
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm h-fit">
              <div className="flex justify-between items-center mb-4">
                ...
                {editingPlayerId && (
                  <button onClick={handleCancelPlayerEdit} className="text-xs text-neutral-500 hover:text-neutral-900 flex items-center gap-1">
                    <X size={14} /> إلغاء التعديل
                  </button>
                )}
              </div>
              ...
              <form onSubmit={handleSubmitPlayer} className="flex flex-col gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-neutral-600 mb-1">صورة اللاعب</label>
                  <input
                    type="file"
                    accept="image/*"
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                {players.length === 0 ? (
                  ...
                  players.map((player) => (
                    <div key={player.id} className="flex items-center justify-between p-4 border border-neutral-100 rounded-lg hover:bg-neutral-50">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-neutral-100 rounded-full flex items-center justify-center overflow-hidden shrink-0 border border-neutral-200 relative">
                          {player.image ? (
                          ...
                          {player.number && (
                            <span className="absolute bottom-0 right-0 bg-neutral-900 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                              {player.number}
                            </span>
                          )}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-xs font-bold text-neutral-900">{player.name}</h4>
                            <span className="bg-neutral-900 text-white text-[10px] px-2 py-0.5 rounded font-medium">{player.position}</span>
                            {player.number && <span className="text-[11px] font-semibold text-neutral-500">#{player.number}</span>}
                          </div>
                          {player.description && <p className="text-[11px] text-neutral-500 mt-1">{player.description}</p>}
                        </div>
                      </div>
